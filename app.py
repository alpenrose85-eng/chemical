...
# Элементы, которые мы хотим видеть в таблице (для каждой стали — свои)
ELEMENTS_BY_STEEL = {
...
def create_word_report(samples):
    ...
    # Собираем все уникальные марки сталей
    used_steels = set(s["steel"] for s in samples)

    for steel in used_steels:
        doc.add_heading(f'Сталь: {steel}', level=1)
    ...
    try:
        ...
        for steel, group_samples in steel_groups.items():
            ...
            if st.button(f"📥 Скачать отчёт для стали {steel}", key=f"download_{steel}"):
                # Создаём отчёт только для этой стали
                filtered_samples = [s for s in samples if s["steel"] == steel]
                doc = Document()
                style = doc.styles['Normal']
                font = style.font
                font.name = 'Times New Roman'
