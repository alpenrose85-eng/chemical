...
def extract_means_from_table(table):
    """Извлекает средние значения из таблицы с двумя блоками (как в вашем файле)."""
    means = {}

    # Проходим по всем строкам таблицы
    for i in range(len(table.rows)):
        first_cell = table.rows[i].cells[0].text.strip()
        # Нормализуем: убираем лишние пробелы и неразрывные символы
        first_cell_clean = re.sub(r'\s+', ' ', first_cell).strip()

        # Ищем строку "Среднее:", но НЕ ту, что начинается с "±"
        if first_cell_clean == "Среднее:":
            # Проверим: если следующая ячейка — не погрешность (не начинается с ±)
            if i + 1 < len(table.rows):
                next_val = table.rows[i].cells[1].text.strip() if len(table.rows[i].cells) > 1 else ""
                if next_val.startswith(("±", "-")):
                    continue  # это строка погрешностей — пропускаем

            # Берём заголовки из строки НАД текущей (обычно это строка с элементами)
            if i > 0:
                headers = []
                for cell in table.rows[i - 1].cells[1:]:
                    h = re.sub(r'\s+', ' ', cell.text).replace('%', '').strip()
                    if h and not h.isdigit() and h != "":
                        headers.append(h)

                # Извлекаем значения
                for j, cell in enumerate(table.rows[i].cells[1:], start=0):
                    if j < len(headers):
                        val_text = re.sub(r'\s+', ' ', cell.text).strip()
                        if val_text and not val_text.startswith(("±", "-")) and val_text != "":
                            try:
...
def parse_protocol_docx(file):
    ...
    while i < len(elements):
        ...
        if typ == 'paragraph' and "Наименование образца" in val:
            ...
            # Ищем первую таблицу после этого блока параграфов
            table = None
            k = j
            ...
            i = k  # пропускаем обработанную таблицу
        i += 1
